class AppConstants{


  static String baseUrl = "https://stagingcrapadvisor.semicolonstech.com/api";
  static String toiletTpeBaseUrl = "https://stagingcrapadvisor.semicolonstech.com/api/getToiletType";

  static String imageBaseUrl = "https://stagingcrapadvisor.semicolonstech.com/public/asset/activity/";
  static String festivalImageBaseUrl = "https://stagingcrapadvisor.semicolonstech.com/public/asset/festivals/";
  static String toiletImageBaseUrl = "https://stagingcrapadvisor.semicolonstech.com/public/asset/toilets/";



static String feedbackEmail = "astraldesignapp@gmail.com";

  static String splashBackground = 'assets/svg/seplash background.svg';
 // static String logo = "assets/svg/logo-crap.svg";
 // static String logo = "assets/svg/newLogo.svg";
  static String logo = "assets/images/finalLogo.jpg";
  static String newLogo = "assets/images/checking.png";
  static String planBackground = "assets/images/planBackground.png";
  static String leftLines = "assets/svg/leftLines.svg";
  static String rightLines = "assets/svg/rightLines.svg";
  static String defaultLoginButton = "assets/svg/defaultLoginButton.svg";
  static String loginButton = "assets/svg/newLoginButton.svg";
  static String backIcon = "assets/svg/greeBackIcon.svg";
  static String crapChat = "assets/images/crapChat.png";
  static String greenBackIcon = "assets/svg/greeBackIcon.svg";
  static String customMarker = "assets/images/customMarker.png";
  static String currentLocationMarker = "assets/images/currentLocationMarker.png";
  static String forwardIcon = "assets/svg/forwardIcon.svg";
  static String submitButton = "assets/svg/newsubmitButton.svg";
  static String premiumBackground = "assets/svg/premiumBackground.svg";
  static String proUnselected = "assets/svg/Pro_Unselected.svg";
  static String profileUnselected = "assets/svg/Profile_Unselected.svg";
  static String homeUnselected = "assets/svg/Home_Unselected.svg";
  static String feedUnselected = "assets/svg/Feed_Unselected.svg";
  static String feedSelected = "assets/svg/Feed_Selected.svg";
  static String homeSelected = "assets/svg/Home_Selected.svg";
  static String proSelected = "assets/svg/Pro_Selected.svg";
  static String profileSelected = "assets/svg/Profile_Selected.svg";
  static String proIcon = "assets/svg/proIcon.svg";
  static String bellIcon = "assets/svg/bellIcon.svg";
  static String crownProIcon = "assets/svg/crownProIcon.svg";
  //static String tile1Background = "assets/svg/tile1background.svg";
  static String tile1Background = "assets/svg/tile1.svg";
 // static String tile2Background = "assets/svg/tile2background.svg";
  static String tile2Background = "assets/svg/tile2.svg";
 // static String tile3Background = "assets/svg/tile3background.svg";
  static String tile3Background = "assets/svg/tile3.svg";
//  static String tile4Background = "assets/svg/tile4background.svg";
  static String tile4Background = "assets/svg/tile4.svg";
 // static String tile5Background = "assets/svg/tile5background.svg";
  static String tile5Background = "assets/svg/tile5.svg";
  //static String tile6Background = "assets/svg/tile6background.svg";
  static String tile6Background = "assets/svg/tile6.svg";
  static String tile1Top = "assets/svg/festivalTop.svg";
  static String tile2Top = "assets/svg/tile2Top.svg";
  static String tile3Top = "assets/svg/tile3Top.svg";
  static String tile4Top = "assets/svg/tile4Top.svg";
  static String tile5Top = "assets/svg/tile5Top.svg";
  static String tile6Top = "assets/svg/tile6Top.svg";
  static String tile7Top = "assets/images/registered.png";
  static String totalFestivals = "assets/svg/totalFestivals.svg";
  static String totalAttendees = "assets/svg/newtotalFestivals.svg";
  static String festivalMangementCardIcon = "assets/svg/festivalManagmentCard.svg";
  static String newsBulletinMangementCardIcon = "assets/svg/newsBulletinCard.svg";
  static String eventMangementCardIcon = "assets/svg/eventManagementCad.svg";
  static String toiletMangementCardIcon = "assets/svg/toiletManagementCard.svg";
  static String kidsMangementCardIcon = "assets/svg/kidsManagementCard.svg";
  static String festivalNameFieldIcon = "assets/svg/festivalNameFieldIcon.svg";
  static String stageRunningOrderCardIcon = "assets/svg/stageRunningOrderCard.svg";
  static String performancesIcon = "assets/svg/performances.svg";
  static String performanceTitleIcon = "assets/svg/performanceTitle.svg";
  static String bandTitleIcon = "assets/svg/bandTitle.svg";
  static String artistTitleIcon = "assets/svg/artistTitle.svg";
  static String specialGuestsTitleIcon = "assets/svg/specialguestsTitle.svg";
  static String addIcon = "assets/svg/addIcon.svg";
  static String timerIcon = "assets/svg/timerIcon.svg";
  static String dropDownPrefixIcon = "assets/svg/dropdownPrefix.svg";
  static String calendarIcon = "assets/svg/calendar.svg";
  static String bulletinTitleIcon = "assets/svg/bulletinTitle.svg";
  static String bulletinContentIcon = "assets/svg/bullerinContent.svg";
  static String timer1Icon = "assets/svg/timer1.svg";


  static String comode1Icon = "assets/svg/comode.svg";
  static String specialAccess = "assets/svg/specialAccess.svg";
  static String ablutionSolution = "assets/svg/ablutionSolution.svg";
  static String standAndDeliver = "assets/svg/standAndDeliver.svg";
  static String composterIsNoImposter = "assets/svg/composterIsNoImposter.svg";
  static String chinaBlueRoal = "assets/svg/china.svg";
  static String longDrop = "assets/svg/longDrop.svg";
  static String femmefetal = "assets/svg/femmefetal.svg";






  static String toiletCategory1Icon = "assets/svg/toiletCategory.svg";
  static String crowdCapicity = "assets/svg/crowdCapacity.svg";
  static String person = "assets/svg/person.svg";
  static String totalAmount = "assets/svg/totalAmount.svg";
  static String tax = "assets/svg/tax.svg";

  static const String email = "assets/svg/email.png";
  static const String web = "assets/svg/web.png";
  static const String facebook = "assets/svg/facebook.png";
  static const String instagram = "assets/svg/instagram.png";
  static const String devices = "assets/svg/devices.png";
  static const String phone = "assets/svg/phone.png";
  static const String member = "assets/svg/member.png";
  static String myProfile = "assets/svg/my_profile.svg";
  static const String contactUS = "assets/svg/contact_us.svg";
  static const String feedBack = "assets/svg/feedback.svg";
  static const String logout = "assets/svg/check-out.png";
  static const String delete = "assets/svg/garbage.png";
  static const String mapPreview = "assets/images/mapPreview.png";












}